...
const PORT  = "1414", LANDING = 'index.html', STATIC_PATH = '/public/';
...
function serverController(hostname) {
    const server = http.createServer((request, response) => {
        ...
        var filePath = path.join(__dirname, STATIC_PATH);       // delivers index.html by default
        if (request.url == '/') {
            var file = path.join(filePath, LANDING);       // delivers index.html by default
            sendFile(file, LANDING, 'text/html', response);
        } else {
            var file = path.join(filePath, request.url);   // delivers requested resource
